...
function get_post_files(posts_directory: string = "_posts"): string[] {
  return readdirSync(posts_directory).map(filename => join(posts_directory, filename));
}
...

        {
          processed_files.map(([title, content]: [string, string]) => {
            ...
            >
              ...
              {/* TODO: Swap the following lines: */}
              <p className={`m-0 text-sm opacity-50`}>
                {content}
              </p>
              {/* with:
          ```
          export default function TextParagraph({ text }) {
